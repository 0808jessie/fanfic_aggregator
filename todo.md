...
- [x] 已依使用者決策延期同人誌中心義忍中文詞組的 live 總數／分頁實測；目前 CAPTCHA／逾時時維持安全隔離與單獨重試
- [x] 依使用者決策，以同人誌中心安全隔離與單獨重試狀態作為本輪公開頁驗證結論並完成交付
- [x] 稽核 CxC 創利市集公開搜尋頁與現有 Adapter、前端平台清單和 URL 白名單契約
- [x] 新增 CxC Adapter、平台狀態與前端篩選／Badge 整合，並維持來源失敗隔離
- [x] 為閱讀清單新增標籤與星級快速篩選，以及 localStorage JSON 匯出與安全匯入
- [x] 讓平台狀態卡可切換搜尋結果的平台快速篩選並維持既有篩選器共存
- [x] 補上 CxC、閱讀清單備份與平台快速篩選測試，完成公開頁與 UI 驗證後儲存 checkpoint
- [x] 已儲存包含 CxC Adapter、閱讀清單備份／篩選、平台狀態卡快速篩選與回歸測試的 checkpoint（eda574b1）
- [x] 稽核 CxC 公開頁渲染選擇器、AO3／在水裡寫字／Penana／同人誌中心的官方總數擷取與現有隔離路徑
- [x] 依公開可驗證資料修正 CxC 渲染等待及各來源 totalWorks 解析，並保持超時或驗證頁安全降級
- [x] 為閱讀清單新增全文搜尋與收藏時間／評分多維排序
- [x] 將 JSON 匯入改為預覽確認彈窗，提供安全合併、完整覆蓋與取消操作
- [x] 新增 CP 詞庫管理 UI：AO3／本地查詢欄位、自訂 localStorage 合併及即時新增
- [x] 補上 Adapter、閱讀清單搜尋排序、匯入預覽與 CP 詞庫管理測試
- [x] 以使用者層級 UI 互動測試載入多筆閱讀清單 JSON，驗證匯入預覽彈窗的統計、作品預覽與合併／覆蓋／取消操作，並保存結果
- [x] 完成匯入預覽彈窗互動驗證後，整體確認閱讀清單與雙欄 CP 詞庫的 UI 驗證紀錄
- [x] 已在完成本輪 UI 與公開來源驗證後儲存 checkpoint（54c458fe）
- [x] 稽核並移除搜尋結果區的全域模糊外部連線錯誤，改由平台狀態列呈現來源專屬訊息與重試
- [x] 強化 CxC 公開頁的回應／卡片有界等待與安全解析，逾時時維持 error 零筆狀態
- [x] 驗證閱讀清單的標題、作者、筆記、標籤全文搜尋與收藏／星級排序控制
- [x] 補上平台狀態、CxC 與閱讀清單操作回歸測試，完成 UI／公開來源驗證並儲存 checkpoint
