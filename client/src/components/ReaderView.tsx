...
export function ReaderView({ work, initialProgress, initialChapterUrl, loadDocument, onClose, onOpenSource, onProgress }: ReaderViewProps) {
  ...
  const tableOfContents = document?.tableOfContents?.length ? document.tableOfContents : document?.chapters[0] ? [{ ...document.chapters[0], url: document?.url, index: 1 }] : [];
  const currentChapterIndex = Math.max(0, Math.min(document?.currentChapterIndex ?? 0, tableOfContents.length - 1));
  const activeChapter = document?.chapters.find((chapter) => chapter.url === document?.url || chapter.url === tableOfContents[currentChapterIndex]?.url) || document?.chapters[0];
  ...
  const loadChapter = useCallback(async (chapterUrl?: string) => {
    ...
    try {
      const preloadedChapter = documentRef.current?.chapters.find((chapter) => chapter.url === targetUrl);
      if (preloadedChapter && documentRef.current) {
        const currentIndex = Math.max(0, (documentRef.current.tableOfContents || []).findIndex((entry) => entry.url === targetUrl));
        const preloadedDocument = { ...documentRef.current, url: targetUrl, currentChapterIndex: currentIndex };
        chapterCacheRef.current.set(targetUrl, preloadedDocument);
        cacheReaderDocument(targetUrl, preloadedDocument);
        setCacheStatus("local");
        documentRef.current = preloadedDocument;
    ...
    catch (reason: unknown) { documentRef.current = null; setDocument(null); setError(reason instanceof Error ? reason.message : "無法載入原始網站的公開內文。"); }
  ...
  useEffect(() => { if (!document || !nextChapter?.url || chapterCacheRef.current.has(nextChapter.url) || document.chapters.some((chapter) => chapter.url === nextChapter.url)) return; const prefetch = loadDocumentRef.current(work.url, nextChapter.url); if (!prefetch || typeof (prefetch as Promise<ReaderDocument>).then !== "function") return; void prefetch.then((prefetched) => { chapterCacheRef.current.set(nextChapter.url!, prefetched); chapterCacheRef.current.set(prefetched.url, prefetched); cacheReaderDocument(nextChapter.url!, prefetched); if (prefetched.url !== nextChapter.url) cacheReaderDocument(prefetched.url, prefetched); }).catch(() => undefined); }, [document, nextChapter?.url, work.url]);
  ...
  const handleTouchEnd = (event: React.TouchEvent<HTMLElement>) => {
    ...
    if (deltaX < 0 && nextChapter) void loadChapter(nextChapter.url || undefined);
    if (deltaX > 0 && previousChapter) void loadChapter(previousChapter.url || undefined);
  };

  return <div role="dialog" aria-modal="true" aria-label={`${work.title} 沉浸閱讀器`} className={`fixed inset-0 z-[80] flex flex-col overscroll-contain ${selectedTheme.surface} ${selectedTheme.ink}`}>
    <header className={`relative z-10 flex items-center justify-between gap-2 border-b ${selectedTheme.line} ${selectedTheme.control} px-4 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur-xl sm:px-6`}>
    ...
    <main ref={viewportRef} onScroll={reportProgress} onTouchStart={(event) => { const touch = event.touches[0]; touchStartRef.current = { x: touch.clientX, y: touch.clientY }; }} onTouchEnd={handleTouchEnd} aria-label="閱讀內容" className={`min-h-0 flex-1 touch-pan-y overflow-auto overscroll-contain ${vertical ? "px-6 py-8" : "px-5 py-10 sm:px-8 sm:py-14"}`}>
