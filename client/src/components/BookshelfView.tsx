import { CheckSquare, Download, FileJson, FileSpreadsheet, FileText, ListFilter, Search, Trash2, Upload } from "lucide-react";
...
export function BookshelfView({ bookmarks, onEdit, onRemove, onImport, onBatchRemove, onBatchUpdate, onProgressChange, onExportAll, onImportAll }: BookshelfViewProps) {
  ...
  return <section className="space-y-4">
    <div className="reader-library-toolbar grid gap-3 p-5 lg:grid-cols-[1.2fr_repeat(4,0.62fr)_auto] lg:items-end">
      <label className="grid gap-2 text-xs font-semibold text-[color:var(--atlas-muted)]">搜尋藏書<div className="relative"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[color:var(--atlas-muted)]" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="標題、作者、標籤或摘要" className="h-10 rounded-xl border-[color:var(--atlas-line)] bg-white/80 pl-9 text-sm" /></div></label>
      <label className="grid gap-2 text-xs font-semibold text-[color:var(--atlas-muted)]">來源<select value={platform} onChange={(event) => setPlatform(event.target.value)} className="h-10 rounded-xl border border-[color:var(--atlas-line)] bg-white/80 px-2 text-sm"><option value="all">全部平台</option>{platforms.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label className="grid gap-2 text-xs font-semibold text-[color:var(--atlas-muted)]">分類<select value={shelf} onChange={(event) => setShelf(event.target.value as BookmarkShelf | "all")} className="h-10 rounded-xl border border-[color:var(--atlas-line)] bg-white/80 px-2 text-sm"><option value="all">全部分類</option><option value="to-read">待讀</option><option value="favorite">最愛</option></select></label>
      <label className="grid gap-2 text-xs font-semibold text-[color:var(--atlas-muted)]">自訂標籤<select value={tag} onChange={(event) => setTag(event.target.value)} className="h-10 rounded-xl border border-[color:var(--atlas-line)] bg-white/80 px-2 text-sm"><option value="">全部標籤</option>{tags.map((item) => <option key={item} value={item}>#{item}</option>)}</select></label>
      <label className="grid gap-2 text-xs font-semibold text-[color:var(--atlas-muted)]">排序<select aria-label="藏書排序" value={sort} onChange={(event) => setSort(event.target.value as BookmarkSort)} className="h-10 rounded-xl border border-[color:var(--atlas-line)] bg-white/80 px-2 text-sm"><option value="saved-desc">收藏時間：新到舊</option><option value="saved-asc">收藏時間：舊到新</option><option value="author">作者名稱</option><option value="words">字數</option><option value="updated">更新時間</option></select></label>
      <div className="flex gap-2"><select aria-label="匯出格式" value={format} onChange={(event) => setFormat(event.target.value as ExportFormat)} className="h-10 rounded-xl border border-[color:var(--atlas-line)] bg-white/80 px-2 text-xs"><option value="json">JSON</option><option value="markdown">Markdown</option><option value="csv">CSV</option></select><Button type="button" onClick={exportList} disabled={!visibleBookmarks.length} className="h-10 rounded-xl bg-[color:var(--atlas-indigo)] px-3 text-xs font-semibold"><Download className="mr-1.5 h-3.5 w-3.5" />匯出</Button><Button type="button" variant="outline" onClick={() => importRef.current?.click()} className="h-10 rounded-xl border-[color:var(--atlas-line)] bg-white/70 px-3"><Upload className="h-3.5 w-3.5" /><span className="sr-only">匯入 JSON 書單</span></Button><input ref={importRef} type="file" accept="application/json,.json" onChange={(event) => importList(event.target.files?.[0])} className="hidden" /></div>
    </div>
    <div className="reader-library-actions flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap items-center gap-2"><Button type="button" variant={selectionMode ? "default" : "outline"} onClick={() => selectionMode ? exitSelection() : setSelectionMode(true)} className={`h-9 text-xs font-semibold ${selectionMode ? "bg-[color:var(--atlas-indigo)]" : "border-0 bg-[color:var(--atlas-elevated)]"}`}><CheckSquare className="mr-1.5 h-3.5 w-3.5" />{selectionMode ? "結束多選" : "批次多選"}</Button>{selectionMode && <><Button type="button" variant="outline" onClick={toggleAllVisible} className="h-9 border-0 bg-[color:var(--atlas-elevated)] text-xs font-semibold">全選目前 {visibleBookmarks.length} 筆</Button><span className="text-xs font-semibold text-[color:var(--atlas-indigo)]">已選 {selectedUrls.size} 筆</span></>}</div><div className="flex items-center gap-2"><Button type="button" variant="outline" onClick={onExportAll} className="h-9 border-0 bg-[color:var(--atlas-indigo-soft)] text-xs font-semibold text-[color:var(--atlas-indigo)]"><FileJson className="mr-1.5 h-3.5 w-3.5" />匯出全部</Button><Button type="button" variant="outline" onClick={() => fullImportRef.current?.click()} className="h-9 border-0 bg-teal-50 text-xs font-semibold text-teal-700"><Upload className="mr-1.5 h-3.5 w-3.5" />完整還原</Button><input ref={fullImportRef} type="file" accept="application/json,.json" onChange={(event) => importAll(event.target.files?.[0])} className="hidden" /></div></div>
    ...
    <SavedBookmarksGrid bookmarks={visibleBookmarks} onEdit={onEdit} onRemove={onRemove} selectionMode={selectionMode} selectedUrls={selectedUrls} onToggleSelected={toggleSelected} onProgressChange={onProgressChange} />
