import { BookOpen, CheckSquare, Download, FileJson, FileSpreadsheet, FileText, LayoutGrid, List, ListFilter, RotateCw, Search, Trash2, Upload } from "lucide-react";
...
import { buildReaderEpub, buildReaderTxt, readerExportFilename, type ReaderExportItem } from "@/lib/readerExport";
import {
  bookmarksToCsv,
  bookmarksToMarkdown,
  filterBookmarks,
...
type BookshelfViewProps = {
  ...
  onRead?: (record: BookmarkRecord) => void;
  onOpenSource?: (url: string) => void;
  loadReaderDocument?: (url: string) => Promise<ReaderDocument>;
  onExportAll: () => void;
...
export function BookshelfView({ bookmarks, onEdit, onRemove, onImport, onBatchRemove, onBatchUpdate, onProgressChange, onRead, onOpenSource, loadReaderDocument, onExportAll, onImportAll, desktopVersion, updateCheckPending = false, onCheckForUpdates, readingHistory = [], onResumeHistory }: BookshelfViewProps) {
  ...
  const [readerExportPending, setReaderExportPending] = useState<"txt" | "epub" | null>(null);
  const [readerExportNotice, setReaderExportNotice] = useState("");
  ...
  const selectedBookmarks = useMemo(() => bookmarks.filter((bookmark) => selectedUrls.has(bookmark.url)), [bookmarks, selectedUrls]);
  ...
  const exportReaderSelection = async (format: "txt" | "epub") => {
    if (!loadReaderDocument || !selectedBookmarks.length || readerExportPending) return;
    setReaderExportPending(format);
    setReaderExportNotice("");
    const collected: ReaderExportItem[] = [];
    const failures: string[] = [];
    for (const bookmark of selectedBookmarks) {
      try {
        const document = await loadReaderDocument(bookmark.url);
        if (!document.chapters.some((chapter) => chapter.paragraphs.some((paragraph) => paragraph.trim()))) throw new Error("來源未提供可匯出的公開正文");
        collected.push({ bookmark, document });
      } catch (error) {
        failures.push(`${bookmark.result.title}：${error instanceof Error ? error.message : "來源暫時不可讀"}`);
      }
    }
    if (collected.length) {
      const now = new Date();
      const payload = format === "txt" ? buildReaderTxt(collected, now) : buildReaderEpub(collected, now);
      const blobPayload: BlobPart = typeof payload === "string" ? payload : new Uint8Array(payload).buffer;
      downloadFile(readerExportFilename(format, now), blobPayload, format === "txt" ? "text/plain" : "application/epub+zip");
      setReaderExportNotice(`已匯出 ${collected.length} 篇${format === "txt" ? " UTF-8 TXT" : " EPUB"}${failures.length ? `；略過 ${failures.length} 篇暫時不可讀來源。` : "。"}`);
    } else {
      setReaderExportNotice("沒有可匯出的公開正文；請改由原始網站閱讀或稍後重試。");
    }
    if (failures.length) console.warn("[ReaderExport] Skipped unavailable public sources:", failures);
    setReaderExportPending(null);
  };

  return <section className="space-y-4">
    {readingHistory.length > 0 && <section aria-label="最近閱讀" className="reader-library-actions p-4"><div className="mb-3 flex min-w-0 items-center justify-between gap-3"><div className="min-w-0"><h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">最近閱讀</h2><p className="mt-1 text-xs text-[color:var(--atlas-muted)]">從上次離開的章節與位置接續閱讀。</p></div><span className="shrink-0 whitespace-nowrap text-xs font-semibold text-[color:var(--atlas-indigo)]">最近 {readingHistory.length} 篇</span></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{readingHistory.slice(0, 6).map((record) => <Button key={record.url} type="button" variant="outline" onClick={() => onResumeHistory?.(record)} className="h-auto min-h-20 w-full justify-start overflow-hidden border-[color:var(--atlas-line)] bg-white/70 px-3 py-3 text-left"><div className="min-w-0 w-full"><div title={record.result.title} className="truncate text-sm font-semibold text-slate-800">{record.result.title}</div><div title={`${record.result.platform} · ${record.chapter || "正文"}`} className="mt-1 truncate text-xs text-[color:var(--atlas-muted)]">{record.result.platform} · {record.chapter || "正文"}</div><div className="mt-2 flex min-w-0 items-center gap-2 text-xs font-semibold text-[color:var(--atlas-indigo)]"><span className="shrink-0 whitespace-nowrap">{record.percent}%</span><span className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-indigo-100"><span className="block h-full bg-[color:var(--atlas-indigo)]" style={{ width: `${record.percent}%` }} /></span></div></div></Button>)}</div></section>}
    ...
    <SavedBookmarksGrid bookmarks={visibleBookmarks} onEdit={onEdit} onRemove={onRemove} selectionMode={selectionMode} selectedUrls={selectedUrls} onToggleSelected={toggleSelected} onProgressChange={onProgressChange} onRead={onRead} onOpenSource={onOpenSource} viewMode={viewMode} />
