...
describe("Tauri updater interaction", () => {
  ...
  it("opens an update dialog at startup and downloads, installs, then relaunches", async () => {
    ...
    updaterState.check.mockResolvedValue({ version: "1.2.3", body: "改用 KadoKado 公開 JSON 搜尋 API", download: updaterState.download, install: updaterState.install });

    render(<Home />);

    await waitFor(() => expect(screen.getByText("發現新版本 v1.2.3")).toBeTruthy());
    expect(screen.getByText("改用 KadoKado 公開 JSON 搜尋 API")).toBeTruthy();
