...
describe("Tauri updater interaction", () => {
  ...
  it("opens an update dialog at startup and downloads, installs, then relaunches", async () => {
    ...
    updaterState.check.mockResolvedValue({ version: "1.2.4", body: "KadoKado 標籤與相關性搜尋品質修正", download: updaterState.download, install: updaterState.install });

    render(<Home />);

    await waitFor(() => expect(screen.getByText("發現新版本 v1.2.4")).toBeTruthy());
    expect(screen.getByText("KadoKado 標籤與相關性搜尋品質修正")).toBeTruthy();
