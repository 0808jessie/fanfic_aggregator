...
describe("Tauri updater interaction", () => {
  ...
  it("opens an update dialog at startup and downloads, installs, then relaunches", async () => {
    ...
    updaterState.check.mockResolvedValue({ version: "1.2.5", body: "搜尋介面精緻化：等高書卡、來源健康收納與 CP 即時提示", download: updaterState.download, install: updaterState.install });

    render(<Home />);

    await waitFor(() => expect(screen.getByText("發現新版本 v1.2.5")).toBeTruthy());
    expect(screen.getByText("搜尋介面精緻化：等高書卡、來源健康收納與 CP 即時提示")).toBeTruthy();
  ...
  it("exposes a manual update check and current desktop version in the bookshelf tools", async () => {
    ...
    expect(screen.getByText("v1.2.5")).toBeTruthy();
