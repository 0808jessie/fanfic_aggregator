...
describe("Home Page Traditional Chinese '義忍' Search", () => {
  it("successfully searches and renders results for traditional '義忍'", async () => {
    ...
    expect(mockState.lastVariables).toEqual({
      path: "/search",
      method: "POST",
      data: { keyword: "Tomioka Giyuu/Kochou Shinobu", platforms: ["ao3", "lofter", "doujin"], page: 1, forceRefresh: false },
    });
    ...
    await waitFor(() => expect(mockState.lastVariables).toEqual({
      path: "/search",
      method: "POST",
      data: { keyword: "Tomioka Giyuu/Kochou Shinobu", platforms: ["ao3", "lofter", "doujin"], page: 1, forceRefresh: true },
    }));
  });
});
