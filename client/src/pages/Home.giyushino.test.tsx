...
describe("Home Page Traditional Chinese '義忍' Search", () => {
  it("successfully searches and renders results for traditional '義忍'", async () => {
    ...
    expect(screen.getByText(/正在查詢 9 個來源/)).toBeTruthy();
    ...
    await waitFor(() => expect(mockState.lastVariables).toMatchObject({
      path: "/search",
      method: "POST",
      data: { keyword: "義忍", mode: "keyword", platforms: ["ao3", "doujin", "waterwriter", "penana", "cxc", "pixiv", "bahamut", "popo", "kadokado"], page: 1, forceRefresh: true, customCpMappings: [] },
    }));
  });
});
