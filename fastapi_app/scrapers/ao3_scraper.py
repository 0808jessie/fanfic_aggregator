...
class AO3Scraper(BaseScraper):
    ...
    def build_search_url(keyword: str, page: int = 1, mode: str = "keyword", language: Optional[str] = None) -> str:
        """Build AO3's native free-text search URL with optional native language filter."""
        parameters = [
            ("commit", "Search"),
        ...
        if language and language != "all":
            # AO3 language_id mapping: zh -> zh, en -> en, ja -> ja
    ...
    def _parse_static_results(self, html: str, keyword: str) -> list[ScrapedFanfic]:
        ...
        for work in soup.select("li.work.blurb"):
            ...
            rating_el = work.select_one("li.rating .text, li.rating")
    ...
    def scrape(
        ...
        try:
            static_payload = self._scrape_static_pages(ao3_query, target_pages, page, mode, language=language)
        finally:
